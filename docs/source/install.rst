...
#. 「mrcube-26.4.2.dmg」ウィンドウの中の「mrcube-26.4.2.app」を「Applications」ディレクトリにドラッグアンドドロップする．
#. 「Applications」ディレクトリの中の「mrcube-26.4.2.app」を実行する．
#. 「Appleは、“mrcube-26.4.2”にMacに損害を与えたり、プライバシーを侵害する可能性のあるマルウェアが含まれていないことを検証できませんでした。」と表示されるため，「完了」をクリックする．
#. 「システム環境設定」から「プライバシーとセキュリティ」を開き，セキュリティの見出し以下にある「お使いのMacを保護するために“mrcube-26.4.2”がブロックされました。」の横の「このまま開く」ボタンをクリックする．
#. 「"mrcube-26.4.2.app"を開きますか？」というダイアログが表示されるため，「このまま開く」をクリックする．
#. 管理者パスワードを入力する．
...
macOS
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
「Applications」ディレクトリの中の「mrcube-26.4.2.app」をゴミ箱に移動する．
